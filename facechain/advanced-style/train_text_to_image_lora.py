...
def log_validation(model_dir, vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch, global_step,
                   **kwargs):
    """
    ...
    if args.template_dir is not None:
        ...
        for jpg, read_jpg, shape, read_mask in zip(jpgs, kwargs['input_images'], kwargs['input_images_shape'],
                                                   kwargs['input_masks']):
            if args.template_mask:
                image = pipeline(
                    args.validation_prompt, image=read_jpg, mask_image=read_mask, strength=0.65,
                    negative_prompt=args.neg_prompt,
                    guidance_scale=args.guidance_scale, num_inference_steps=20, generator=generator,
                    height=kwargs['new_size'][1], width=kwargs['new_size'][0],
                ).images[0]
            else:
                image = pipeline(
        ...
        for _ in range(args.num_validation_images):
            images.append(
                pipeline(args.validation_prompt, negative_prompt=args.neg_prompt, guidance_scale=args.guidance_scale, \
                         num_inference_steps=50, generator=generator, height=args.resolution,
...
def main():
    ...
    model_dir = snapshot_download(args.pretrained_model_name_or_path, revision=args.revision,user_agent={'invoked_by': 'trainer', 'third_party': 'facechain'})
